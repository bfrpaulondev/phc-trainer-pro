...
const DEFAULT_ORIGINS = ["https://bfrpaulondev.github.io"];
...
export default {
  async fetch(request, env) {
    ...
    if (!cors.ok) {
      return json({ error: "Origem não permitida: " + (origin || "(sem Origin)") }, 403, cors.headers);
    }
    ...
    if (rateLimited(ip)) {
      return json({ error: "Rate limit do proxy excedido (" + RATE_MAX_PER_MIN + "/min)" }, 429, cors.headers);
    }

    let body;
    try { body = await request.json(); }
    catch (e) { return json({ error: "JSON inválido no corpo do pedido" }, 400, cors.headers); }
    ...
    if (!key) return json({ error: "Segredo " + up.keyVar + " não configurado no Worker" }, 500, cors.headers);
    ...
    try {
      ...
      return json({ error: "Falha no upstream: " + String((e && e.message) || e) }, 502, cors.headers);
    }
  }
};
